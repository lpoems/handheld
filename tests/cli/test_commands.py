...
def _patch_handheld_serve_runtime(monkeypatch, config: Config, seen: dict[str, object]) -> None:
    pytest.importorskip("aiohttp")

    class _FakeHandheldApp:
        pass
    ...
    def _fake_create_app(agent_loop, *, model_name: str, auth_token: str, log_buffer_size: int = 200):
        seen["agent_loop"] = agent_loop
        seen["model_name"] = model_name
        seen["auth_token"] = auth_token
        seen["log_buffer_size"] = log_buffer_size
        return _FakeHandheldApp()
    ...
    monkeypatch.setattr("nanobot.agent.loop.AgentLoop", _FakeAgentLoop)
    monkeypatch.setattr("nanobot.handheld.server.create_app", _fake_create_app)
    monkeypatch.setattr("aiohttp.web.run_app", _fake_run_app)
...
def test_handheld_serve_uses_workspace_override_and_cli_network_options(
    monkeypatch, tmp_path: Path
) -> None:
    config_file = _write_instance_config(tmp_path)
    config = Config()
    ...
    _patch_handheld_serve_runtime(monkeypatch, config, seen)
    ...
    assert seen["workspace"] == override_workspace
    assert seen["host"] == "127.0.0.1"
    assert seen["port"] == 18799
    assert seen["auth_token"] == "secret-token"
